/**
 * Integration tests for @internal/e2e-util workspace scaffold utilities
 *
...
describe('Workspace Scaffold Integration', () => {
  ...
  describe('createWorkspace', () => {
    ...
    }, 120000); // 2 minutes timeout for workspace creation

    it('should create a workspace with custom name', async () => {
    ...
    it('should create a workspace with specified number of libraries', async () => {
      const workspace = await createWorkspace({
        libs: 3,
        baseDir: testBaseDir,
      });
    ...
    it('should create a workspace with an application', async () => {
      const workspace = await createWorkspace({
        libs: 1,
        includeApp: true,
        baseDir: testBaseDir,
      ...
      if (!workspace.app) {
        throw new Error('Expected app to be created');
      }
    ...
    it('should create workspace with short path to avoid OS limits', async () => {
      const workspace = await createWorkspace({ baseDir: testBaseDir });
      createdWorkspaces.push(workspace.path);
  ...
  describe('addSourceFile', () => {
    it('should add a source file to a project', async () => {
      const workspace = await createWorkspace({
        libs: 1,
        baseDir: testBaseDir,
      });
    ...
    it('should handle multiple files in the same project', async () => {
      const workspace = await createWorkspace({
        libs: 1,
        baseDir: testBaseDir,
      });
  ...
  describe('getProjectImportAlias', () => {
    it('should get the import alias for a library', async () => {
      const workspace = await createWorkspace({
        libs: 2,
        baseDir: testBaseDir,
      });
