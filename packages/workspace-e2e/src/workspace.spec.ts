...
// Test generator option scenarios to ensure all option flags produce expected behavior
describe('move-file generator option scenarios', () => {
  let projectDirectory: string;
  ...
  describe('MOVE-PROJECT-DIR: projectDirectory option', () => {
    it('should create target at specified directory path', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      // Create and export a utility file
      const utilPath = join(projectDirectory, lib1, 'src', 'lib', 'util.ts');
      writeFileSync(utilPath, 'export function util() { return 42; }\n');

      const indexPath = join(projectDirectory, lib1, 'src', 'index.ts');
      writeFileSync(indexPath, "export * from './lib/util';\n");

      // Move to explicit subdirectory
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --project-directory features/utils --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
      ...
      const lib2IndexPath = join(projectDirectory, lib2, 'src', 'index.ts');
      const lib2IndexContent = readFileSync(lib2IndexPath, 'utf-8');
      expect(lib2IndexContent).toContain('./lib/features/utils/util');

      // Assert: Source project index no longer exports it
      const lib1IndexContent = readFileSync(indexPath, 'utf-8');
      expect(lib1IndexContent).not.toContain('util');
    });
  });

  describe('MOVE-DERIVE-DIR: deriveProjectDirectory option', () => {
    it('should derive target directory from source path', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      writeFileSync(utilPath, 'export function authUtil() { return true; }\n');

      // Move with derived directory
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/features/auth/auth-util.ts --project ${lib2} --derive-project-directory --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
      ...
      expect(readFileSync(movedPath, 'utf-8')).toContain('authUtil');
    });

    it('should fail when both projectDirectory and deriveProjectDirectory are set', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      // Assert: Command fails with mutually exclusive options
      expect(() => {
        execSync(
          `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --project-directory foo --derive-project-directory --no-interactive`,
  ...
  describe('MOVE-SKIP-EXPORT: skipExport option', () => {
    it('should not update index file when skipExport is true', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      // Read original lib2 index
      const lib2IndexPath = join(projectDirectory, lib2, 'src', 'index.ts');
      const originalLib2Index = readFileSync(lib2IndexPath, 'utf-8');

      // Move with skipExport
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --skip-export --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
      ...
      const newLib2Index = readFileSync(lib2IndexPath, 'utf-8');
      expect(newLib2Index).toBe(originalLib2Index);
      expect(newLib2Index).not.toContain('util');
  ...
  describe('MOVE-SKIP-FORMAT: skipFormat option', () => {
    it('should preserve unformatted content when skipFormat is true', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      writeFileSync(utilPath, unformattedContent);

      // Move with skipFormat
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --skip-format --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
  ...
  describe('MOVE-UNICODE: allowUnicode option', () => {
    it('should allow Unicode characters in filenames when enabled', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      execSync(
        `npx nx generate @nxworker/workspace:move-file "${lib1}/src/lib/${unicodeFileName}" --project ${lib2} --allow-unicode --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
      ...
      expect(readFileSync(movedPath, 'utf-8')).toContain('unicodeUtil');

      // Assert: Imports updated in consumer (now cross-project)
      const lib2Alias = getProjectImportAlias(projectDirectory, lib2);
      const consumerContent = readFileSync(consumerPath, 'utf-8');
      expect(consumerContent).toContain(lib2Alias);
    });

    it('should reject Unicode filenames when allowUnicode is false (default)', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      const unicodeFileName = 'util-émoji.ts';
      const unicodePath = join(
        projectDirectory,
        lib1,
        'src',
      ...
      writeFileSync(unicodePath, 'export function util() { return 1; }\n');

      // Assert: Move fails without allowUnicode
      expect(() => {
        execSync(
          `npx nx generate @nxworker/workspace:move-file "${lib1}/src/lib/${unicodeFileName}" --project ${lib2} --no-interactive`,
          { cwd: projectDirectory, stdio: 'pipe' },
        );
      }).toThrow();
    });
  ...
  describe('MOVE-REMOVE-EMPTY: removeEmptyProject option', () => {
    it('should remove source project when it becomes empty', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --remove-empty-project --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );
      ...
      const projectJsonPath = join(projectDirectory, lib1, 'project.json');
      expect(() => readFileSync(projectJsonPath, 'utf-8')).toThrow();
    });

    it('should not remove source project when removeEmptyProject is false', () => {
      const lib1 = uniqueId('lib1-');
      const lib2 = uniqueId('lib2-');
      ...
      // Move without removeEmptyProject (default false)
      execSync(
        `npx nx generate @nxworker/workspace:move-file ${lib1}/src/lib/util.ts --project ${lib2} --no-interactive`,
        { cwd: projectDirectory, stdio: 'inherit' },
      );

      // Assert: Source project still exists
      const projectJsonPath = join(projectDirectory, lib1, 'project.json');
      expect(() => readFileSync(projectJsonPath, 'utf-8')).not.toThrow();
    });
  });
});
