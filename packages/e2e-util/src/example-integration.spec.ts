/**
 * Example integration test demonstrating harness utilities usage
 *
 * This test shows how to use the harness utilities in a real scenario.
 * It's designed to be run manually or in CI to verify the harness works correctly.
 */
...
describe('harness utilities integration example', () => {
  ...
  describeIntegration('end-to-end workflow', () => {
    ...
    it('should create workspace, install plugin, and run generator', async () => {
      ...
      const lib1Alias = getProjectImportAlias(
        workspace.path,
        workspace.libs[0],
      );
