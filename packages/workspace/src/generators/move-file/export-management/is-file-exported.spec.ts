...
describe('isFileExported', () => {
  ...
  it('should detect export * from pattern', () => {
    ...
    expect(
      isFileExported(tree, project, 'lib/utils.ts', cachedTreeExists),
    ).toBe(true);
  });
  ...
  it('should match files without extension', () => {
    ...
    expect(
      isFileExported(tree, project, 'lib/utils.tsx', cachedTreeExists),
    ).toBe(true);
    expect(
      isFileExported(tree, project, 'lib/utils.js', cachedTreeExists),
    ).toBe(true);
  });
  ...
  it('should not match partial file names', () => {
    tree.write('libs/mylib/src/index.ts', `export * from './lib/utils';\n`);
    expect(isFileExported(tree, project, 'lib/util.ts', cachedTreeExists)).toBe(
      false,
    );
  });
  ...
  it('should handle nested paths', () => {
    ...
    expect(
      isFileExported(tree, project, 'lib/nested/utils.ts', cachedTreeExists),
    ).toBe(true);
  });
  ...
  it('should detect exports in multiple entrypoints', () => {
    ...
    expect(
      isFileExported(tree, project, 'lib/other.ts', cachedTreeExists),
    ).toBe(true);
  });
  ...
  it('should handle multiple exports in same file', () => {
    ...
    expect(
      isFileExported(tree, project, 'lib/helpers.ts', cachedTreeExists),
    ).toBe(true);
    expect(
      isFileExported(tree, project, 'lib/other.ts', cachedTreeExists),
    ).toBe(false);
